...
def saveFddbData(path, file_name):
    ...
    for item in file_list:
        ...
        if not ('/' in item):
            continue
        image_num += 1
        im = cv2.imread(path+ 'originalPics/' +item+'.jpg')
        if im is None:
        ...
        for i, (box) in enumerate(boxes):
            ...
            y1 = box[1]*h
            y2 = box[3]*h
            f_write.write(str(x1.item())+'\t'+str(y1.item())+'\t'+str(x2.item()-x1.item())+'\t'+str(y2.item()-y1.item())+'\t'+str(probs[i].item())+'\t'+'1\n')
    f_write.close()
...
if __name__ == '__main__':
    ...
    path = '/home/xxh/Desktop/learning_pytorch/faceboxes-master/picture/FDDB/'
    file_name = 'fddb.txt'
